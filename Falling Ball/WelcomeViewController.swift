...
class WelcomeViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, UITextFieldDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let thisScore = userSettings.highScores[indexPath.row]
        
        let cell = UITableViewCell(style: .value1 , reuseIdentifier: "highScoreCell")
        cell.textLabel?.text = "\(indexPath.row + 1). \(thisScore.name)"
        cell.detailTextLabel?.text = "\(thisScore.score)"
        return cell
    }
    ...
    func textFieldDidChangeSelection(_ textField: UITextField) {
        playButton.isEnabled = textField.text!.count > 0
    }
}
